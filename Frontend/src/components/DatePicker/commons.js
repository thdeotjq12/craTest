...
export const formatDate = (dateObj, dateFormat) =>
...
export const formatDateString = (mode, dates, dateFormat) => {
  if (!Array.isArray(dates) || !dates || (dates && dates.length === 0))
    return "";
  switch (mode) {
    case "single": {
      const dateObj =
        typeof dates[0] === "string"
          ? parseDate(dates[0], dateFormat)
          : dates[0];
    ...
    case "multiple": {
      return [].map
        .call(dates, date => {
          const dateObj =
            typeof date === "string" ? parseDate(date, dateFormat) : date;
          return formatDate(dateObj, dateFormat);
        })
        .join(", ");
    }
    case "range": {
      ...
      const endDateObj =
        typeof dates[1] === "string"
          ? parseDate(dates[1], dateFormat)
          : dates[1];
      return `${formatDate(startDateObj, dateFormat)} to ${formatDate(
        endDateObj,
        dateFormat
      )}`;
    }
